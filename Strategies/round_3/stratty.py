...
class Trader:
    ...
    def __init__(self) -> None:
        ...
        self.basket_mean_difference = 411.9
        self.basket_std = 33.389996883675984
        self.basket_std_factor = 0.55
    ...
    def run(self, state: TradingState):
        ...
        for product in state.order_depths:
            ...
            if product == 'ORCHIDS':
                ...
                orchids_orders += self.calculate_orders(product, order_depth, lower_bound, upper_bound, orchids=True)
            ...
            elif product == 'CHOCOLATE' or product == 'STRAWBERRIES' or product == 'ROSES':
                other_product_orders = self.compute_other_products(state, product, self.position_limit[product], order_depth)
                result[product] = other_product_orders
    ...
    def compute_other_products(self, state: TradingState, product, product_position_limit, product_order_depth):
        orders = []

        if not product_order_depth:
            return orders
        
        # calculate the current inventory
        current_inventory = state.position.get(product, 0)
        inventory_factor = current_inventory / product_position_limit

        # calculate the mid price
        best_ask_price = min(product_order_depth.sell_orders.keys())
        best_bid_price = max(product_order_depth.buy_orders.keys())
        mid_price = (best_ask_price + best_bid_price) / 2

        # calculate order book imbalance
        total_bid_volume = sum(product_order_depth.buy_orders.values())
        total_ask_volume = sum(product_order_depth.sell_orders.values())
        book_imbalance = 0
        if total_bid_volume + total_ask_volume > 0:
            book_imbalance = (total_bid_volume - total_ask_volume) / (total_bid_volume + total_ask_volume)

        # Incorporate inventory factor and book imbalance into spread calculation
        target_spread = mid_price * (0.001 + 0.001 * inventory_factor - 0.0005 * book_imbalance)
        bid_price = mid_price - target_spread
        ask_price = mid_price + target_spread

        if len(product_order_depth.sell_orders) != 0:
            best_ask, best_ask_amount = list(product_order_depth.sell_orders.items())[0]

            ask_size = max(1, min(product_position_limit - current_inventory, best_ask_amount))
            if ask_price > best_ask:
                orders.append(Order(product, best_ask, best_ask_amount))

        if len(product_order_depth.buy_orders) != 0:
            best_bid, best_bid_amount = list(product_order_depth.buy_orders.items())[0]

            bid_size = max(1, min(current_inventory + product_position_limit, best_bid_amount))
    ...
    def rolling_regression_and_momentum(self, product, state):
        prices = self.last_prices.get(product, [])
        if len(prices) >= 4:
            x = np.arange(len(prices[-4:]))
            y = np.array(prices[-4:])  # Last 4 prices
            slope, intercept = np.polyfit(x, y, 1)
            predicted_price = (slope * (len(prices) + 1)) + intercept
            return predicted_price
        return None

    def generate_orders_based_on_prediction(self, product, state, predicted_price):
        if predicted_price is None:
            return []
        current_price = self.get_current_price(state, product)
        if current_price is None:
            return []

        orders = []
        price_difference = predicted_price - current_price
        signal_threshold = current_price * 0.01  # Example threshold: 1% of current price

        if abs(price_difference) > signal_threshold:
            signal = 'BUY' if price_difference > 0 else 'SELL'
            quantity = min(10, int(abs(price_difference) / signal_threshold * 10), self.position_limit[product] - abs(self.POSITION[product]))
            
            if signal == 'BUY':
                price_to_place = min(state.order_depths[product].sell_orders) - 1
                orders.append(Order(product, price_to_place, quantity))
            elif signal == 'SELL' and self.POSITION[product] > 0:
                price_to_place = max(state.order_depths[product].buy_orders) + 1
                orders.append(Order(product, price_to_place, -min(quantity, self.POSITION[product])))

        for order in orders:
            if order.quantity > 0:
                self.POSITION[product] += order.quantity
            else:
                self.POSITION[product] += order.quantity  # Subtract since it's a sell order
    ...
    def compute_basket_trades(self, state):
        ...
        for item in basket_products:
            ...
            mid_prices[item] = (best_sell_price + best_buy_price) / 2
        ...
        if diff > self.basket_std_factor * self.basket_std:
            basket_orders += self.calculate_orchids_orders("GIFT_BASKET", order_depth, -int(1e9), worst_bid)
        elif diff < -self.basket_std_factor * self.basket_std:
            basket_orders += self.calculate_orchids_orders("GIFT_BASKET", order_depth, worst_ask, int(1e9))
    ...
    def calculate_orders(self, product, order_depth, our_bid_price, our_ask_price, orchids=False, gift_basket=False):
        orders: List[Order] = []
