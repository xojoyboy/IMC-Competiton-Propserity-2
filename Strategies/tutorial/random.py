    ...
    def algorithm_mean_reversion(self, symbol: str, state: TradingState, mean: int):
        ...
        while exit_flag < 2:
            ...
            for bid_price, bid_quantity in symbol_sell_order_depth:
                if bid_price <= mean - gap and current_position < self.position_limit[symbol]:
                    max_buy_quantity = min(-bid_quantity * liquidity_adjustment, self.position_limit[symbol] - current_position)
                    orders.append(Order(symbol, bid_price, int(max_buy_quantity)))
                    current_position += int(max_buy_quantity)
                    symbol_sell_order_depth.pop(0)
                    exit_flag = 0
                else:
                    break  # Exit the loop if no more buying opportunity
            ...
            for ask_price, ask_quantity in symbol_buy_order_depth:
                if ask_price >= mean + gap and current_position > -self.position_limit[symbol]:
                    max_sell_quantity = min(ask_quantity * liquidity_adjustment, current_position)
                    orders.append(Order(symbol, ask_price, int(-max_sell_quantity)))  # Note the negative quantity for selling
                    current_position -= int(max_sell_quantity)
                    symbol_buy_order_depth.pop(0)
                    exit_flag = 0
                else:
                    break  # Exit the loop if no more selling opportunity
